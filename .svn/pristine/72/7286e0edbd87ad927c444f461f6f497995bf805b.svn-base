...
>({
  ...
  const syncedValuesKey = JSON.stringify(syncedValues ?? {});

  useEffect(() => {
    Object.entries(syncedValues ?? {}).forEach(([name, value]) => {
      setValue(name as Path<TValues>, value as TValues[Path<TValues>], {
        shouldDirty: false,
      });
    });
  }, [setValue, syncedValuesKey]);
