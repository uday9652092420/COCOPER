...
const SUPPLIER_PAYMENT_PAGE_SIZE = 10
...
const SupplierPaymentPage: React.FC = () => {
  ...
  const totalPages = Math.ceil(filtered.length / SUPPLIER_PAYMENT_PAGE_SIZE)
  const safeCurrentPage = Math.min(currentPage, Math.max(totalPages, 1))
  const paginatedRecords = filtered.slice(
    (safeCurrentPage - 1) * SUPPLIER_PAYMENT_PAGE_SIZE,
    safeCurrentPage * SUPPLIER_PAYMENT_PAGE_SIZE,
  )
