...
const CUSTOMER_RECEIPT_PAGE_SIZE = 10
...
const CustomerReceiptPage: React.FC = () => {
  ...
  const totalPages = Math.ceil(filtered.length / CUSTOMER_RECEIPT_PAGE_SIZE)
  const safeCurrentPage = Math.min(currentPage, Math.max(totalPages, 1))
  const paginatedRecords = filtered.slice(
    (safeCurrentPage - 1) * CUSTOMER_RECEIPT_PAGE_SIZE,
    safeCurrentPage * CUSTOMER_RECEIPT_PAGE_SIZE,
  )
