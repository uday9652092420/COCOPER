...
const PurchaseInvoiceModal: React.FC<{
   ...
   */
  const recalcLine = (index: number, modeOverride?: 'tonage' | 'lessing', piecesModeOverride?: boolean, applyDefault = false, syncPercentageDiscount = false) => {
    ...
    const percentageDiscount = activePiecesMode
      ...
      : discount
    const calculatedDiscount = activePiecesMode && (discount === 0 || syncPercentageDiscount) ? percentageDiscount : discount
    ...
    if (activePiecesMode && (syncPercentageDiscount || ((piecesModeOverride !== undefined || applyDefault) && discount === 0)) && percentageDiscount > 0) {
      setValue(`lines.${index}.discount`, String(Number(percentageDiscount.toFixed(6))))
    }
                  ...
                  <label className="inline-flex items-center gap-2">
                    <input
                      ...
                      onChange={() => {
                        setMode('lessing')
                        setPiecesMode(false)
                        ;(linesField.fields || []).forEach((_, idx) => recalcLine(idx, 'lessing', false))
                      }}
                    />
                    <span>Lessing</span>
                  </label>
              ...
              <table className="min-w-full text-left text-[11px]">
                <thead className="bg-slate-50 text-slate-500">
                  <tr>
                    ...
                    {mode !== 'lessing' && piecesMode ? <th className="px-3 py-2">Pieces %</th> : null}
                      ...
                      <td className="px-3 py-1.5">
                        <input
                          ...
                          {...register(`lines.${index}.quantity` as const, {
                            onChange: () => recalcLine(index, undefined, undefined, true),
                          })}
                        ...
                        <>
                          <td className="px-3 py-1.5">
                            <input
                              ...
                              {...register(`lines.${index}.piecesPercentage` as const, { onChange: () => recalcLine(index, undefined, undefined, true, true) })}
                            />
                          </td>
                        </>
                      ) : (
...
const PurchaseInvoicePage: React.FC = () => {
  ...
  * Format: `<FirstLetterOfOrgName>PI-<NN>` e.g. "Maiprosoft" -> MPI-01.
  ...
  const totalPages = Math.ceil(filtered.length / PURCHASE_INVOICE_PAGE_SIZE)
  const safeCurrentPage = Math.min(currentPage, Math.max(totalPages, 1))
  const paginatedRecords = filtered.slice(
    (safeCurrentPage - 1) * PURCHASE_INVOICE_PAGE_SIZE,
    safeCurrentPage * PURCHASE_INVOICE_PAGE_SIZE,
  )
...
<body>
  ...
  return (
    <div>
      ...
      <DataGrid<PurchaseInvoice> data={paginatedRecords} columns={visibleColumns} getRowId={(row) => row.id} loading={loading} />
