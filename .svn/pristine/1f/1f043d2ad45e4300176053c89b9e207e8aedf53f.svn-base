...
const DirectSalesModal: React.FC<{
   ...
   */
  const recalcLine = (index: number, modeOverride: 'tonage' | 'lessing' = mode, piecesModeOverride?: boolean, applyDefault = false, syncPercentageDiscount = false) => {
    ...
    const percentageDiscount = activePiecesMode
      ? (quantity * Math.min(Math.max(piecesPercentage, 0), 100)) / 100
      : Number(line.discount) || 0
    const enteredDiscount = Number(line.discount) || 0
    const discount = activePiecesMode && (enteredDiscount === 0 || syncPercentageDiscount) ? percentageDiscount : enteredDiscount
    ...
    if (activePiecesMode && (syncPercentageDiscount || ((piecesModeOverride !== undefined || applyDefault) && enteredDiscount === 0)) && percentageDiscount > 0) {
      setValue(`lines.${index}.discount`, Number.isFinite(percentageDiscount) ? Number(percentageDiscount.toFixed(6)) : 0)
    }
        ...
        <form onSubmit={handleSubmit(submit)} className="grid max-h-[80vh] grid-rows-[auto,1fr,auto] gap-3 overflow-y-auto px-5 py-4 text-xs">
          ...
          {!selectedSalesOrderNo && <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-2 text-[11px]">
            ...
            <div className="flex items-center gap-3 rounded-2xl border border-slate-100 bg-slate-50/70 px-3 py-2">
              ...
              <label className="inline-flex items-center gap-2"><input type="radio" name="ds-mode" disabled={isReadOnly} checked={mode === 'lessing'} onChange={() => { if (isReadOnly) return; setMode('lessing'); setPiecesMode(false); linesField.fields.forEach((_, idx) => recalcLine(idx, 'lessing', false)) }} /><span>Lessing</span></label>
            </div>
              ...
              <table className="min-w-full text-left text-[11px]">
                <thead className="bg-slate-50 text-slate-500">
                  <tr>
                    ...
                    {mode !== 'lessing' && piecesMode && <th className="px-3 py-2">Pieces %</th>}
                ...
                <tbody>
                  {linesField.fields.map((field, index) => (
                    <tr key={field.id} className="border-t border-slate-100">
                      ...
                      <td className="px-3 py-1.5">
                        <input
                          ...
                          {...register(`lines.${index}.quantity` as const, {
                            valueAsNumber: true,
                            onChange: () => recalcLine(index, mode, undefined, true),
                          })}
                        />
                      </td>
                      {mode !== 'lessing' && piecesMode ? (
                        <>
                          <td className="px-3 py-1.5">
                            <input type="number" min="0" max="100" step="any" disabled={Boolean(selectedSalesOrderNo) || isApproved} className="w-16 rounded-full border border-slate-200 px-2 py-1 disabled:bg-slate-100" {...register(`lines.${index}.piecesPercentage` as const, { valueAsNumber: true, onChange: () => recalcLine(index, mode, undefined, true, true) })} />
                          </td>
                        </>
                      ) : (
                        null
                      ...
                      <td className="px-3 py-1.5">
                        <input type="number" disabled={Boolean(selectedSalesOrderNo) || isApproved} className="w-20 rounded-full border border-slate-200 px-2 py-1 disabled:bg-slate-100" {...register(`lines.${index}.discount` as const, { valueAsNumber: true, onChange: () => recalcLine(index) })} />
                      </td>
...
const DirectSalesPage: React.FC = () => {
  ...
  const totalPages = Math.ceil(filtered.length / DIRECT_SALES_PAGE_SIZE)
  const safeCurrentPage = Math.min(currentPage, Math.max(totalPages, 1))
  const paginatedRecords = filtered.slice(
    (safeCurrentPage - 1) * DIRECT_SALES_PAGE_SIZE,
    safeCurrentPage * DIRECT_SALES_PAGE_SIZE,
  )
