...
const PURCHASE_ORDER_PAGE_SIZE = 10
...
const PurchaseOrderModal: React.FC<{
      ...
      return {
        ...
        lines:
          existing.lines?.map((l) => ({
            ...
            discount: String(roundValue(Number(l.discount ?? (existing.mode === 'tonagePercentage'
  ...
  const discountLabel = mode === 'tonagePercentage'
    ...
    : mode === 'tonage'
      ? 'Discount (Kgs)'
      : 'Discount (Pieces)'
  const discountColumnLabel = mode === 'tonagePercentage' ? 'Discount (Kgs)' : discountLabel
   ...
   */
  const recalcLine = (index: number, modeOverride?: 'tonage' | 'tonagePercentage' | 'lessing', applyDefault = false, syncPercentageDiscount = false) => {
    ...
    const percentageDiscount = (quantity * Math.min(Math.max(piecesPercentage, 0), 100)) / 100
    const effectiveDiscount = activeMode === 'tonagePercentage' && (discount === 0 || syncPercentageDiscount) ? percentageDiscount : discount

    let actualQuantity = 0
    if (activeMode === 'tonagePercentage') {
      actualQuantity = quantity - effectiveDiscount
    ...
    if (activeMode === 'tonagePercentage' && (syncPercentageDiscount || ((modeOverride !== undefined || applyDefault) && discount === 0)) && percentageDiscount > 0) {
      setValue(`lines.${index}.discount`, String(roundValue(percentageDiscount, 0)))
    }
   ...
   */
  const recalcSalesLine = (index: number, modeOverride?: 'tonage' | 'lessing', applyDefault = false, syncPercentageDiscount = false) => {
    ...
    if (activeMode === 'tonagePercentage' && (syncPercentageDiscount || ((modeOverride !== undefined || applyDefault) && discount === 0)) && percentageDiscount > 0) {
      setValueS(`lines.${index}.discount`, String(Number(percentageDiscount.toFixed(6))))
    }
                      ...
                      <tr key={field.id} className="border-t border-slate-100">
                        ...
                        <td className="px-3 py-1.5">
                            ...
                            {...register(`lines.${index}.quantity` as const, {
                              onChange: () => recalcLine(index, undefined, true),
                            })}
                        ...
                        <td className="px-3 py-1.5">
                          <input
                            ...
                            {...register(`lines.${index}.${mode === 'tonagePercentage' ? 'piecesPercentage' : 'discount'}` as const, {
                              onChange: () => recalcLine(index, undefined, mode === 'tonagePercentage', mode === 'tonagePercentage'),
                            })}
                          />
                        </td>
                      ...
                      {mode === 'tonagePercentage' && (
                        <div>
                          ...
                          <input
                            ...
                            inputMode="decimal"
                            disabled={isPurchaseLocked}
                            className="w-full rounded-full border border-slate-200 px-3 py-1 disabled:cursor-not-allowed disabled:bg-slate-100"
                            {...register(`lines.${index}.discount` as const, { onChange: () => recalcLine(index) })}
                          />
                        </div>
                      )}
                  ...
                  <table className="min-w-full text-left text-[11px] rounded-2xl border border-slate-100">
                    <thead className="bg-[#e8f4c8] text-[#00534f]">
                      <tr>
                        ...
                        <th className="px-3 py-2">{salesMode === 'lessing' ? 'Quantity (Pieces)' : 'Quantity (Tons)'}</th>
                        {salesMode === 'tonagePercentage' ? <th className="px-3 py-2">Pieces %</th> : null}
                        <th className="px-3 py-2">{salesMode === 'tonagePercentage' ? 'Discount (Kgs)' : salesMode === 'tonage' ? 'Discount (Kgs)' : 'Discount (Pieces)'}</th>
                        <th className="px-3 py-2">Actual Quantity</th>
                      ...
                      {salesFields.map((field, index) => {
                        return (
                          <tr key={field.id} className="border-t border-slate-100">
                            ...
                            <td className="px-3 py-1.5">
                              <input type="text" inputMode="decimal" disabled className="w-20 rounded-full border border-slate-200 bg-[#e8f4c8] px-2 py-1" {...registerS(`lines.${index}.quantity` as const, { onChange: () => recalcSalesLine(index, undefined, true) })} />
                            </td>

                            {salesMode === 'tonagePercentage' ? (
                              <>
                                <td className="px-3 py-1.5">
                                  <input type="text" inputMode="decimal" disabled={isSalesLocked} className="w-16 rounded-full border border-slate-200 px-2 py-1 disabled:cursor-not-allowed disabled:bg-[#e8f4c8]" {...registerS(`lines.${index}.piecesPercentage` as const, { onChange: () => recalcSalesLine(index, undefined, true, true) })} />
                                </td>
                                <td className="px-3 py-1.5">
...
const PurchaseOrderPage: React.FC = () => {
  ...
  * Format: `<FirstLetterOfOrgName>PO-<NN>` e.g. "Maiprosoft" -> MPO-01.
  ...
  const totalPages = Math.ceil(filtered.length / PURCHASE_ORDER_PAGE_SIZE)
  const safeCurrentPage = Math.min(currentPage, Math.max(totalPages, 1))
  const paginatedRecords = filtered.slice(
    (safeCurrentPage - 1) * PURCHASE_ORDER_PAGE_SIZE,
    safeCurrentPage * PURCHASE_ORDER_PAGE_SIZE,
  )
  ...
  const printPurchaseOrder = (row: PurchaseOrder) => {
    ...
    const supplierRecord = suppliers.find((s) => s.id === row.supplierId)
    const supplier = resolveSupplierName(row.supplierId)
    const supplierAddress = [supplierRecord?.address, supplierRecord?.state]
      .filter(Boolean)
      .join(', ')
  ...
  return (
    <div>
      ...
      <div className="mt-3 rounded-3xl border border-emerald-100 bg-emerald-50/30 p-3 shadow-sm">
        ...
        <DataGrid<PurchaseOrder> data={paginatedRecords} columns={visibleColumns} getRowId={(row) => row.id} loading={loading} />
