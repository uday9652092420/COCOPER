 ...
 * @description User Branches maintenance screen.
 */
...
const BranchesMasterPage: React.FC = () => {
  ...
  const filtered = useMemo(
    () =>
      records.filter((b) => {
        ...
        return !q || b.branch_name.toLowerCase().includes(q) || (b.branch_code ?? '').toLowerCase().includes(q)
      }),
    [records, search]
  )
  ...
  return (
    <div>
      <PageHeader title="User Branches" breadcrumb={['Masters', 'User Branches']} />
      <Toolbar title="User Branches" onAdd={() => { setEditing(null); setModalOpen(true) }} />
      ...
      <MasterFormModal<BranchFormValues>
        ...
        defaultValues={
          editing
            ? { branchCode: editing.branch_code ?? '', branchName: editing.branch_name, address: editing.address ?? '', contactNo: editing.contact_no ?? '', status: (editing.status as 'ACTIVE' | 'INACTIVE') || 'ACTIVE' }
            : { branchCode: '', branchName: '', address: '', contactNo: '', status: 'ACTIVE' }
        }
