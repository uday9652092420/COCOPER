...
/**
 * @component LoginPage
 * @description Login page component with validation and dummy authentication.
 */
      ...
      <div className="grid w-full max-w-4xl items-center gap-8 rounded-3xl bg-white/10 p-4 shadow-2xl backdrop-blur-lg md:grid-cols-2 md:p-8">
        <div className="hidden flex-col gap-4 text-white md:flex">
          <div className="flex items-center gap-3">
            <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-white/10 text-lg font-bold shadow-lg">
              CO
            </div>
          ...
          <p className="mt-4 text-sm leading-relaxed text-emerald-50">
            Manage warehouses, suppliers, customers, purchases, sales and dispatches in a
            single modern, lightweight application designed exclusively for coconut
            wholesalers.
          </p>
          <div className="mt-4 grid grid-cols-2 gap-3 text-xs text-emerald-50">
            <div className="rounded-2xl bg-white/10 p-3">
              <p className="text-[11px] font-semibold">Real-time Overview</p>
              <p className="mt-1 text-[11px]">Dashboard cards and charts keep you on top of daily purchases and sales.</p>
            </div>
            <div className="rounded-2xl bg-white/10 p-3">
              <p className="text-[11px] font-semibold">Smart Transactions</p>
              <p className="mt-1 text-[11px]">Business rules for customer credit, OT, indirect sales and dispatches.</p>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-3xl bg-white/95 p-5 shadow-xl md:p-7">
          <div className="mb-4 flex items-center gap-3 md:hidden">
            <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-gradient-to-br from-emerald-500 to-lime-400 text-sm font-bold text-white shadow-lg">
              CO
            </div>
            <div className="flex flex-col">
              <span className="text-base font-semibold text-slate-900">COCOS</span>
              <span className="text-[11px] text-slate-500">Coconut Wholesale Management System</span>
            </div>
          </div>

          <h2 className="text-lg font-semibold text-slate-900">Welcome back</h2>
          <p className="mb-4 mt-1 text-xs text-slate-500">Sign in with your administrator credentials.</p>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="flex items-center justify-between text-[11px]">
              <label className="inline-flex items-center gap-1 text-slate-600">
                <input type="checkbox" className="h-3 w-3 rounded border-slate-300" {...register('rememberMe')} />
                Remember me
              </label>
              <button type="button" className="text-[11px] font-medium text-[#2E7D32] hover:underline">
                Forgot password?
              </button>
            </div>

            {error ? <p className="rounded-full bg-rose-50 px-3 py-1 text-[11px] text-rose-600">{error}</p> : null}
